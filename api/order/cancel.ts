...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    findUrl.searchParams.set("name", orderNumber.startsWith("#") ? orderNumber : `#${orderNumber}`);
    if (email) findUrl.searchParams.set("email", String(email));
    const fr = await fetch(findUrl.toString(), {
      headers: { "X-Shopify-Access-Token": TOKEN, "Content-Type": "application/json" }
    });
    if (!fr.ok) return res.status(fr.status).json({ error: await fr.text() });
    const data: any = await fr.json();
    const order = data?.orders?.[0];
    if (!order) return res.json({ success: false, message: "Order not found" });

    const fs = order.fulfillment_status; // null | fulfilled | partial
    if (fs && fs !== "null") {
